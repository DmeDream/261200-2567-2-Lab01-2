...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener {
    ...
    public static void main(String[] a) {
        ...
        for (int i = 0; i < 360; i++) {
            bob.penColor("darkgreen");
            bob.forward(1);        // ระยะเดินไปข้างหน้า
            bob.left(1);           // หมุนซ้ายทีละ 1 องศา
        }
        bob.width(37);
        ...
        bob.left(90);
         ...
         //วาดกลีบดอกไม้รอบ ๆ
            ...
            bob.left(90);
            bob.forward(55);
        for (int i = 0; i < 12; i++) { // กลีบ 12
            drawLeaf(bob);
            bob.left(30); // หมุน 30 องศาสำหรับแต่ละกลีบ
        }


        // วาดเทียนตรงกลาง
        bob.up();
        bob.setPosition(0, 50); // ไปตำแหน่งตรงกลางบนกระทง
//        bob.left(90);
        bob.down();
        bob.penColor("yellow");
        bob.width(5);
        bob.forward(50); // วาดเส้นตรงสำหรับเทียน

        // วาดเปลวไฟ
        bob.penColor("yellow");
        bob.width(2);
        for (int i = 0; i < 36; i++) { // วาดเปลวไฟเป็นวงโค้ง
            bob.forward(1);
            bob.left(10);
        }
    }

    public static void drawLeaf(Turtle bob) {
        bob.down();
        for (int i = 0; i < 2; i++) {
            bob.penColor("orange");
            bob.forward(30); // ด้านหนึ่งของกลีบ
            bob.left(60);
            bob.forward(30);
            bob.left(120);
        }
